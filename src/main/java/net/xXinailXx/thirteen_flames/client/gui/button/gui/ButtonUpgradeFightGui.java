...
public class ButtonUpgradeFightGui extends AbstractWidget {
    ...
    public ButtonUpgradeFightGui(int x, int y) {
        super(x, y, 46, 48);
    }

    public void onPress() {
        if (guiLevelingData.getFightLevel(MC.player) < 100 && !guiLevelingData.isPlayerScreen(MC.player)) {
            if (scarabsData.getScarabGold(MC.player) >= 1) {
                scarabsData.addScarabGold(MC.player, -1);
                guiLevelingData.addFightLevel(MC.player, 1);
            }
        }
    }
    ...
    public void renderButton(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (scarabsData.getScarabGold(MC.player) >= 1 && guiLevelingData.getFightLevel(MC.player) < 100 && !guiLevelingData.isPlayerScreen(MC.player))
