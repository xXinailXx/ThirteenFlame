...
public class ButtonUpgradeCraftGui extends AbstractWidget {
    ...
    public void onPress() {
        if (guiLevelingData.getCraftLevel(MC.player) < 100 && !guiLevelingData.isPlayerScreen(MC.player)) {
            if (scarabsData.getScarabGold(MC.player) >= 1) {
                scarabsData.addScarabGold(MC.player, -1);
                guiLevelingData.addCraftLevel(MC.player, 1);
            }
        }
    }
    ...
    public void renderButton(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (scarabsData.getScarabGold(MC.player) >= 1 && guiLevelingData.getCraftLevel(MC.player) < 100 && !guiLevelingData.isPlayerScreen(MC.player))
