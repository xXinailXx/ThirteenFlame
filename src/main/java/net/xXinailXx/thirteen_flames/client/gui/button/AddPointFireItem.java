package net.xXinailXx.thirteen_flames.client.gui.button;
...
import net.xXinailXx.thirteen_flames.mixin.client.AddXPLightItemRelicScreenMixin;
...
public class AddPointFireItem extends AbstractDescriptionWidget implements IHoverableWidget {
    ...
    private RelicDescriptionScreen screen1;
    private AddXPLightItemRelicScreenMixin screen;
    private static final IData.IScarabsData scarabsData = new Data.ScarabsData();
    private Player player = MC.player;
    private final ItemStack stack;

    public AddPointFireItem(int x, int y, RelicDescriptionScreen screen, ItemStack stack) {
        super(x, y, 83, 29);
        this.screen1 = screen;
        this.stack = stack;
    }
        ...
        if (item instanceof RelicItem relic) {
            ...
            if (!(isMaxLevelRelic)) {
                if (scarabsData.getScarabSilver(this.player) >= 1) {
                    ...
                    LevelingUtils.addExperience(this.player, item.getDefaultInstance(), 50);
                }
            }
        }
    }
    ...
    public void onHovered(PoseStack poseStack, int i, int i1) {
        ...
        if (item instanceof RelicItem relic) {
            ...
            if (!(isMaxLevelRelic)) {
                entries.add(Component.translatable("gui.st_thirteen_lights.relic.button_add_point.add_level").append(Component.literal(String.valueOf(scarabsSilver))));
            } else {
                entries.add(Component.translatable("gui.st_thirteen_lights.relic.button_add_point.max_level"));
            }
