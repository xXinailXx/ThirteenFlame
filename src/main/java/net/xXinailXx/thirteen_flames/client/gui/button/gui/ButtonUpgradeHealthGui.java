...
public class ButtonUpgradeHealthGui extends AbstractWidgetUtils {
    ...
    public void onPress() {
        if (guiLevelingData.getGuiHealthLevelAmount() < 100 && !guiLevelingData.isPlayerScreen()) {
            if (scarabsData.getScarabGold() >= 1) {
                scarabsData.addScarabGold(-1);
                guiLevelingData.addGuiHealthLevelAmount(1);
            }
        }
    }
    ...
    public void renderButton(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (scarabsData.getScarabGold() >= 1 && guiLevelingData.getGuiHealthLevelAmount() < 100 && !guiLevelingData.isPlayerScreen())
