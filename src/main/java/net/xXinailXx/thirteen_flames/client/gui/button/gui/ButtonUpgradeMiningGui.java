...
public class ButtonUpgradeMiningGui extends AbstractWidgetUtils {
    ...
    public void onPress() {
        if (guiLevelingData.getGuiMiningLevelAmount() < 100 && !guiLevelingData.isPlayerScreen()) {
            if (scarabsData.getScarabGold() >= 1) {
                scarabsData.addScarabGold(-1);
                guiLevelingData.addGuiMiningLevelAmount(1);
            }
        }
    }
    ...
    public void renderButton(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (scarabsData.getScarabGold() >= 1 && guiLevelingData.getGuiMiningLevelAmount() < 100 && !guiLevelingData.isPlayerScreen()) {
