...
public class ButtonUpgradeMiningGui extends AbstractWidgetUtils {
    ...
    public ButtonUpgradeMiningGui(int x, int y) {
        super(x, y, 46, 48);
    }

    public void onPress() {
        if (guiLevelingData.getMiningLevel(MC.player) < 100 && !guiLevelingData.isPlayerScreen(MC.player)) {
            if (scarabsData.getScarabGold(MC.player) >= 1) {
                scarabsData.addScarabGold(MC.player, -1);
                guiLevelingData.addMiningLevel(MC.player, 1);
            }
        }
    }
    ...
    public void renderButton(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (scarabsData.getScarabGold(MC.player) >= 1 && guiLevelingData.getMiningLevel(MC.player) < 100 && !guiLevelingData.isPlayerScreen(MC.player))
